...
namespace LiveResults.Model
{
    public class EmmaApiClient : IDisposable, IEmmaClient
    {
        ...
        private void FireResultChanged(Runner r, int position)
        {
            ...
            if (!string.IsNullOrEmpty(ConfigurationManager.AppSettings["serverapipollurl"]))
            {
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        {
                            new KeyValuePair<string, string>("key", ConfigurationManager.AppSettings["serverapipollkey"]),
                        });
                        string resp = client.PostAsync(ConfigurationManager.AppSettings["serverapipollurl"], content)
                            .GetAwaiter().GetResult()
                            .Content.ReadAsStringAsync()
                            .GetAwaiter().GetResult();
                        if (resp.Trim().Length > 0)
                        {
                            foreach (string line in resp.Trim().Split('\n'))
                            {
                                var s = new EmmaApiServer
                                {
                                    Host = line
                ...
                {
                    System.Windows.Forms.MessageBox.Show("Could not connect to " + new Uri(ConfigurationManager.AppSettings["serverapipollurl"]).Host + " to query connection, error was: " + ee.Message + "\r\n\r\nStacktrace: " + ee.StackTrace);
                }
            }
        ...
        private readonly string m_server_url;
        ...
        public EmmaApiClient(string server, int competitionID, bool assignIDsInternally = false)
        {
            ...
            m_server_url = server;
            m_httpClient = new HttpClient { BaseAddress = new Uri(server) };
        }
        ...
        public void SetRadioControl(string className, int code, string controlName, int order)
        {
            if (!m_classRadioControls.ContainsKey(className))
                m_classRadioControls.Add(className, Array.Empty<RadioControl>());
        ...
        public int UpdatesPending => m_itemsToUpdate.Count;
