    ...
    public class EmmaApiClient : IDisposable, IEmmaClient
    {
        ...
        public async System.Threading.Tasks.Task Start()
        {
            ...
            try
            {
                ...
                HttpResponseMessage response = await client.GetAsync("adm/uploadApi.php?comp=" + m_compID + "&method=getcompetitionresultdata");
                            ...
                            else if (item is Runner)
                            {
                                ...
                                if (r.ResultUpdated)
                                {
                                    var formContent = new FormUrlEncodedContent(new[]
                                    {
                                        ...
                                        new KeyValuePair<string, string>("finishTime", r.FinishTime == null ? "" : r.FinishTime.Value.ToString("%Y-%MM-%dd %H:%mm:%ss")),
                                    });
                                    var response = await client.PostAsync("/adm/uploadApi.php", formContent);
                                    response.EnsureSuccessStatusCode();
