...
<head><title><?=$_TITLE?></title>
    ...
    <meta name="description"
          content="Följ svensk och internationell orientering live på nätet med hjälp av liveresultat">
                        ...
                        <td>
                            ...
                            <table border="0" cellpadding="0" cellspacing="0" width="100%" id="tblLiveComps">
								...
								foreach ($comps as $comp) {
									?>
                                    <tr id="row<?=$comp["tavid"]?>" style="font-size:12px;font-weight:bold;">
                                        <td><?=date("Y-m-d", strtotime($comp['compDate']))?></td>
                                        <td><a onmouseover="colorRow('row<?=$comp["tavid"]?>')"
                                               onmouseout="resetRow('row<?=$comp["tavid"]?>')"
                                               href="followfull.php?comp=<?=$comp['tavid']?>&amp;lang=<?=$lang?>"><?=$comp["compName"]?></a>
                                        </td>
